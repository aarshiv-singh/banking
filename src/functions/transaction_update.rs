...
pub fn transaction_update(
    ...
    let data_path = path_variables::get_main_path();
    let transaction_path = path_variables::get_transaction_path(user);
    let mut data_csv = csv_function::data_reader(&data_path).unwrap_or_else(|error| {
        panic!("Error in getting main data from csv with error : {}", error);
    });
    let mut transaction_csv =
        csv_function::transaction_reader(&transaction_path).unwrap_or_else(|error| {
            panic!(
                "Error in getting transaction data from csv with error: {}",
                error
            );
        });
    ...
    for mut record in &mut data_csv {
        if record.user_name.eq(user) {
            ...
            let new_transaction = structures::build_transaction(
    ...
    csv_function::data_writer(&data_path, data_csv).unwrap_or_else(|error| {
        panic!("Cannot write to the data file csv with error: {}", error);
    });

    csv_function::transaction_writer(&transaction_path, transaction_csv).unwrap_or_else(|error| {
        panic!(
            "Cannot write to the transaction file csv with error: {}",
            error
        );
    });
